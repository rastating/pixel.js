...
function PixelJS() {
    ...
    this._sprites = [];
    this._tiles = [];
    this._transparentLayer = undefined;
    this._transparentLayerCtx = undefined;
...
PixelJS.prototype.init = function (args) {
    ...
    this._transparentLayer = document.createElement('canvas');
    this._transparentLayer.width = this._canvas.width;
    this._transparentLayer.height = this._canvas.height;
    this._transparentLayerCtx = this._transparentLayer.getContext('2d');
    ...
    (function loop(elapsedTime) {
        ...
        if (!isNaN(self._deltaTime)) {
            for (var i = 0; i < self._components.length; i++) {
                self._components[i].update(elapsedTime, self._deltaTime);
            }
...
PixelJS.prototype.createPlayer = function() {
    var player = new this.Player(this);
    this._components.push(player);
    return player;
};

PixelJS.prototype.displayFPS = function() {
    if (this._fpsCounter != undefined) {
        this._fpsCounter = new this.FPSCounter(this);
        this._components.push(this._fpsCounter);
    }
};

PixelJS.prototype.flip = function() {
...
PixelJS.prototype.drawImage = function(data, x, y) {
    this._backBufferCtx.putImageData(data, x, y);
};

PixelJS.prototype.drawTransparentImage = function(data, x, y) {
    this._transparentLayerCtx.clearRect(0, 0, this._transparentLayer.width, this._transparentLayer.height);
    this._transparentLayerCtx.putImageData(data, x, y);
    this._backBufferCtx.drawImage(this._transparentLayer, 0, 0);
};

PixelJS.prototype.drawFromCanvas = function(canvas, x, y) {
