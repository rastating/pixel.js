...
PixelJS.prototype.Player = function (layer) {
    var self = this;
    this._sprite = new layer.engine.AnimatedSprite(layer);
    ...
    this.layer.engine.on('keydown', function (keyCode) {
        switch (keyCode) {
            case self.layer.engine.Keys.Left:
                self.direction |= self.layer.engine.Directions.Left;
                break;
                
            case self.layer.engine.Keys.Up:
                self.direction |= self.layer.engine.Directions.Up;
                break;
                
            case self.layer.engine.Keys.Right:
                self.direction |= self.layer.engine.Directions.Right;
                break;
                
            case self.layer.engine.Keys.Down:
                self.direction |= self.layer.engine.Directions.Down;
                break;
        }
...
PixelJS.prototype.Player.prototype.update = function (elapsedTime, dt) {
    if (this.allowDiagonalMovement) {
        if ((this.direction & this.layer.engine.Directions.Right) != 0) {
            this._sprite.moveRight();
        }
        
        if ((this.direction & this.layer.engine.Directions.Left) != 0) {
            this._sprite.moveLeft();
        }
        
        if ((this.direction & this.layer.engine.Directions.Up) != 0) {
            this._sprite.moveUp();
        }
        
        if ((this.direction & this.layer.engine.Directions.Down) != 0) {
        ...
        else if ((this.direction & this.layer.engine.Directions.Up) != 0) {
            this._sprite.row = this._directionRowMap.up;
            this._sprite._isAnimating = true;
            this._sprite.moveUp();
        }
        else if ((this.direction & this.layer.engine.Directions.Left) != 0) {
            this._sprite.row = this._directionRowMap.left;
            this._sprite._isAnimating = true;
            this._sprite.moveLeft();
        }
        else if ((this.direction & this.layer.engine.Directions.Down) != 0) {
...
PixelJS.prototype.Player.prototype.draw = function () {
    this._sprite.draw();
};