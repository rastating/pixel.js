...
PixelJS.prototype.AnimatedSprite = function (layer) {
    this._loaded = false;
...
PixelJS.prototype.AnimatedSprite.prototype.load = function (info, callback) {
    var self = this;
    this.spriteSheet = this.layer.createSpriteSheet();
...
PixelJS.prototype.AnimatedSprite.prototype.draw = function (args) {
    ...
    if (this._loaded) {
        if (this.animationSpeed > 0 && this._isAnimating) {
            if (!isNaN(this.layer.engine._deltaTime)) {
                this._frameTickCount += (this.layer.engine._deltaTime / 1000);
        ...
        this.layer.drawImage(this.spriteSheet._frameImages[this.row][this._currentFrame], this.pos.x, this.pos.y);
    }
};